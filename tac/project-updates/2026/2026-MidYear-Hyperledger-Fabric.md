...
1. **Contribution volume declined.** Non-bot commits on `main` fell from 79 in H1 2025 to 64 in H1 2026, and distinct non-bot authors fell from 32 to 23 over the same comparison. The project lists nine core maintainers, but the trend is downward, not flat; the separate LFDT lifecycle test concerns maintaining organizations and is addressed below.
...
One health concern stands out: **maintainer concentration.** The orderer and the committer carry most of the commit volume but list only two maintainers each. The 2026 annual review characterized Fabric-X maintainers as primarily from IBM Research, but current maintainer files do not record organizations, so this report cannot independently verify the LFDT maintaining-organization threshold. Notably, the wider ecosystem has broader contributor participation — Fabric Smart Client had 16 distinct contributors in the window against 4 on the committer — which suggests a pool the core repositories have not yet drawn on.
...
8. **Maintainer growth and mentorship** (orderer and committer maintainers): add maintainers to the orderer and committer beyond two each, formalise onboarding and a maintainer-promotion path, and establish a named mentorship program or documented equivalent. Maintainer growth alone would not complete the mentorship-program goal that did not start in H1.
...
- Contributors with high-performance distributed systems experience, especially for the orderer and committer, where maintainer counts are at two each.
...
The annual review characterized Fabric-X maintainers as primarily from IBM Research, which originally developed the architecture, but current maintainer files do not record organizations. Combined with two-maintainer counts on the highest-traffic repositories, this concentration remains the sub-project's clearest community-health risk. The Fabric-X TSC should document affiliations so the TAC can verify the Incubation threshold of at least two maintaining organizations.

## Project Lifecycle Status Recommendation

- **Hyperledger Fabric: Graduated, subject to affiliation verification.** No change is recommended based on release cadence, maintenance activity, and nine listed core maintainers. However, LFDT criteria require at least three maintaining organizations for Graduated status, and current maintainer files do not record affiliations. The TAC should confirm that requirement rather than infer compliance from maintainer count. The declining contributor trend remains an H2 concern.
- **Hyperledger Fabric-X: Incubation, subject to affiliation verification.** No change. The v1.0.0 milestones, documentation work, Kubernetes and observability delivery through the Ansible collection, and growth to 11 maintainers are strong signals. Remaining concerns include no project-maintained adopter evidence, an experimental Fabric Smart Client driver, two-maintainer counts on the busiest repositories, unverified organizational diversity, and inconsistent release publishing. The TAC should confirm that Fabric-X meets the Incubation requirement of at least two maintaining organizations.
